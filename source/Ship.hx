...
class Ship extends FlxSprite
{
	...
	public var definition(default, null):Array<Array<Int>>;
	...
	public function generate(?Seed:UInt=-1,?ColorSeed:UInt=-1):Void
	{
		...
		definition = new Array<Array<Int>>();
		definition = [];
		for (r in 0...ROWS)
		{
			definition[r] = [];
		...
		for (i in 0...avoidcs.length)
		{
			...
			definition[r][c] = ((_seed & bitmask) != 0) ? AVOID : EMPTY;
			bitmask <<= 1;
		}
		var emptycs = [4,5,4,4];
		var emptyrs = [6,6,7,8];
		...
		for (i in 0...emptycs.length)
		{
			c = emptycs[i];
			r = emptyrs[i];
			definition[r][c] = ((_seed & bitmask) != 0) ? SOLID : COKPT;
			bitmask <<= 1;
		}
		...
		for (r in 0...ROWS)
		{
			for (c in 0...Std.int(COLS/2))
			{
				...
				if (definition[r][c] == EMPTY)
				{
					needsolid = false;
					if ((c > 0) && (definition[r][c - 1] == AVOID)) 
						needsolid = true;
					if ((c < (COLS/2) - 1) && (definition[r][c + 1] == AVOID)) 
						needsolid = true;
					if ((r > 0) && (definition[r - 1][c] == AVOID)) 
						needsolid = true;
					if ((r < ROWS - 1) && (definition[r + 1][c] == AVOID)) 
						needsolid = true;
					if (needsolid)
						definition[r][c] = SOLID;
				}
		...
		for (r in 0...ROWS)
		{
			for (c in 0...Std.int(COLS / 2))
			{
				definition[r][COLS - 1 - c] = definition[r][c];
			}
		}
